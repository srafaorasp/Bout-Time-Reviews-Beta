...
export function initializeApp() {
    ...
    if (!isLoaded) {
        state.universeId = generateUniverseId();
    ...
    try {
        ...
        if (savedData) {
            ...
            if (parsedData.universeFighters && parsedData.roster) {
                state.universeId = parsedData.universeId || generateUniverseId();
                state.universeFighters = parsedData.universeFighters;
                state.universeFighters.forEach(f => {
                    if (!f.universeId) {
                        f.universeId = state.universeId;
                    }
                });
...
export function attachEventListeners() {
    ...
    dom.cards.item1.importBtn.addEventListener('click', () => handleImport('item1'));
    dom.cards.item2.importBtn.addEventListener('click', () => handleImport('item2'));
    ...
    dom.fighterInfoModal.saveBtn.addEventListener('click', () => { 
        ...
        populateAndShowFighterModal(fighter);
    });
    ...
    dom.universeSetupModal.importBtn.addEventListener('click', () => {
        triggerFileUpload((data) => {
            if (data.roster && data.universeFighters) {
                state.universeId = data.universeId || generateUniverseId();
